    ...
    FeatureSpec({
            ...
            scenario("Load TSL from the internet") {
                ...
                val ca =
                    tsl.caServices.first {
                        it.env == TrustEnvironment.PU &&
                            it.name ==
                            "CN=D-Trust.SMCB-CA3,OU=Institution des Gesundheitswesens-CA der Telematikinfrastruktur,O=D-TRUST GmbH,C=DE"
                    }
                ca shouldNotBe null
            }
        }
        ...
        feature("OCSP Requests") {
            ...
            val pkiClient =
                PKIClient {
                    loader =
                        { ListOfTrustedServiceLists(pu = ListOfTrustedServiceLists.loadFromServer(HttpClient(CIO), TrustEnvironment.TU)) }
                }
