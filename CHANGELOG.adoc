...
- Unterstützung der OCSP-Abfragen für X509-Zertifikate (s. --ocsp option by `vzd-cli admin list` und `vzd-cli admin cert-list`
- Neues Befehl: `vzd-cli admin dump` lädt große Mengen von Einträgen vom Server für nachfolgende loikale Verarbeitung
(z.B. Datenanalysen)
- Es werden mehr Zertifikatdetails angezeigt (z.B. einzelne Subject-Felder, OCSP-Responcer-URL)
