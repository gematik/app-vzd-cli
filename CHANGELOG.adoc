...
- `vzd-cli admin <ENV> login` wurde komplett überarbeitet
- `vzd-cli admin <ENV> token` ersetzt `vzd-cli admin auth`
- Neuer Befehl: `vzd-cli admin <ENV> search`: schnelle, intuitive Suche nach Einträgen
- Neuer Befehl: `vzd-cli admin <ENV> show <TELEMATIK_ID>`: zeigt den gesamten Eintrag in verschiedenen Formaten
- Neuer Befehl: `vzd-cli admin <ENV> edit <TELEMATIK_ID>`: editieren eines Basiseintrags in einem Text-Editor
