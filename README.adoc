...
.Abmelden in der Referenzumgebung, Client-Credentials werden über Parameter übergeben
[source,bash]
----
vzd-cli admin -e ru login-cred --client-id=myclient --secret=mysecret
----

.Abmelden in der Produktivumgebung, Client-Id wird über Parameter übergeben, Client-Secret wird aus der Umgebungsvariable `CLIENT_SECRET` ausgelesen.
[source,bash]
----
...
# TelematikID und BasisEintrag werden automatisch aus dem Zertifikat ermittelt (Admission Statement -> Registration Number)
