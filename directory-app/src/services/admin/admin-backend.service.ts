...
import {
  ...
  DirectoryEntryFHIRResourceType,
  DirectoryEntryKind,
  ElaborateSearchResults,
  BaseDirectoryEntry,
  Outcome,
} from './admin.model';
...
export class AdminBackendService {
  public status$: ReplaySubject<AdminStatus> = new ReplaySubject<AdminStatus>(
    1,
  );
  ...
  loadEntry(
    env: string,
    telematikID: string,
  ): Promise<ElaborateDirectoryEntry> {
    return firstValueFrom(
  ...
  getEntryKindIcon(entry: ElaborateDirectoryEntry): string {
    if (
      entry.base.fhirResourceType == DirectoryEntryFHIRResourceType.Practitioner
    ) {
      return 'user';
    } else {
      return 'hospital';
  ...
  modifyBaseEntry(
    env: string,
    baseEntry: BaseDirectoryEntry,
  ): Promise<BaseDirectoryEntry> {
    return firstValueFrom(
      this.http
        .put<BaseDirectoryEntry>(
